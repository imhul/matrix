...
import type { Symbol, Chain, Matrix as MatrixType } from './types'

export class Matrix {
    app: PIXI.Application;
    matrix: MatrixType = [];
    pathCache: Map<string, PIXI.Graphics> = new Map();
    columnTextures: PIXI.RenderTexture[] = [];
    ...
    createColumn(col: number) {
        ...
        unit.chain.forEach((symbolIndex, j) => {
            ...
            const s = new PIXI.Sprite(this.getSymbolTexture(g));

            s.scale.set(symbolScale);
            s.x = col * symbolSize;
            s.y = unit.y + j * (symbolSize + 15);

            s.tint = isFirstSymbol ? PIXI.Color.fromHex(firstSymbolColor) : PIXI.Color.fromHex(matrixColor);
    ...
    getSymbolTexture = (graphic: PIXI.Graphics): PIXI.RenderTexture => {
    ...
    update = () => {
        for (let col = 0; col < this.matrix.length; col++) {
            const unit = this.matrix[col];
            const column = this.container.children[col] as PIXI.Container;
            ...
            for (let j = 0; j < column.children.length; j++) {
                const sprite = column.children[j] as PIXI.Sprite;
