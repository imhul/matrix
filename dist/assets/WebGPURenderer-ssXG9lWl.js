...
class W {
	...
	destroyAll() {
		var e
		for (const t in this._gpuBuffers) (e = this._gpuBuffers[t]) == null || e.destroy()
		this._gpuBuffers = {}
	}
	createGPUBuffer(e) {
		...
		return (
			(e._updateID = 0),
			e.data && (ae(e.data.buffer, t.getMappedRange()), t.unmap()),
...
class Y {
	constructor(e) {
		...
		for (let r = 0; r < t; r++) {
			...
			;(r === 0 && (s |= T.COPY_SRC),
				this._buffers.push(new ue({ data: this._batchBuffer.data, usage: s })))
		}
	}
	renderEnd() {
	...
	_getBufferResource(e) {
		if (!this._bufferResources[e]) {
			...
			this._bufferResources[e] = new xe({ buffer: t, offset: ((e / 2) | 0) * 256, size: b })
		}
		return this._bufferResources[e]
	}
	_getBindGroup(e) {
		if (!this._bindGroups[e]) {
			const t = new L({ 0: this._getBufferResource(e) })
...
class ke {
	...
	getDescriptor(e, t, r) {
		...
		if (
			...
			const c = t & y.STENCIL ? 'clear' : 'load',
				l = t & y.DEPTH ? 'clear' : 'load'
			o = {
...
const De = {
		...
		upload(a, e, t) {
			...
			t.device.queue.writeTexture(
				{ texture: e },
				r,
				{ offset: 0, rowsPerImage: a.pixelHeight, bytesPerRow: a.pixelWidth * i },
		...
		for (let u = 0; u < s; ++u) {
			...
			for (let h = 1; h < e.mipLevelCount; ++h) {
				const f = r.createView({
					...
					d = n.beginRenderPass({
						colorAttachments: [
							{ view: f, storeOp: 'store', loadOp: 'clear', clearValue: { r: 0, g: 0, b: 0, a: 0 } }
						]
					}),
...
class te {
	constructor(e) {
		;((this.managedTextures = []),
			...
			(this._uploads = { image: ee, buffer: De, video: Fe, compressed: ze }),
			(this._renderer = e),
	...
	_createTextureBindGroup(e) {
		const t = e.source
		return (
			(this._bindGroupHash[e.uid] = new L({
				0: t,
				1: t.style,
				2: new k({ uTextureMatrix: { type: 'mat3x3<f32>', value: e.textureMatrix.mapCoord } })
			})),
			this._bindGroupHash[e.uid]
		)
	}
...
class re {
	...
	contextChange(e) {
		...
		const r = H({ name: 'graphics', bits: [he, pe(this._maxTextures), ge, z] })
		this.shader = new F({ gpuProgram: r, resources: { localUniforms: t } })
	}
	execute(e, t) {
		const r = t.context,
		...
		for (let _ = 0; _ < u.instructionSize; _++) {
			...
			if (
				...
				const P = m.textures
				;((m.bindGroup = A(P.textures, P.count, this._maxTextures)),
...
class se {
	init() {
		const e = H({ name: 'mesh', bits: [me, Pe, z] })
		this._shader = new F({
			gpuProgram: e,
			resources: {
