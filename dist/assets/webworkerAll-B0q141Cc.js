import {
	...
	e as C,
	as as Pt
} from './index-HrDsaxoc.js'
import { c as q, a as Ft, b as Mt, B as De } from './colorToUniform-BXaCBwVl.js'
class Oe {
	static init(e) {
...
class Ie {
	static init(e) {
		;((e = Object.assign({ autoStart: !0, sharedTicker: !1 }, e)),
			Object.defineProperty(this, 'ticker', {
				...
				set(t) {
					;(this._ticker && this._ticker.remove(this.render, this),
						(this._ticker = t),
						t && t.add(this.render, this, ht.LOW))
				},
				get() {
					return this._ticker
				}
			...
			(this._ticker = null),
			(this.ticker = e.sharedTicker ? fe.shared : new fe()),
...
class Rt extends ft {
	...
	get font() {
		return (
			v(T, 'BitmapFont.font is deprecated, please use BitmapFont.fontFamily instead.'),
			this.fontFamily
		)
	}
	get pageTextures() {
		return (
			v(T, 'BitmapFont.pageTextures is deprecated, please use BitmapFont.pages instead.'),
			this.pages
		)
	}
	get size() {
		return (
			v(T, 'BitmapFont.size is deprecated, please use BitmapFont.fontMetrics.fontSize instead.'),
	...
	destroy(e = !1) {
		...
		;((this.chars = null),
			e && (this.pages.forEach((r) => r.texture.destroy(!0)), (this.pages = null)))
	}
}
const We = class Ee extends Rt {
	constructor(e) {
		;(super(),
			(this.resolution = 1),
			(this.pages = []),
		...
		;(t.overrideSize
			...
			t.textureStyle &&
				(this._textureStyle = t.textureStyle instanceof W ? t.textureStyle : new W(t.textureStyle)),
			(this.fontMetrics = O.measureFont(n)),
			(this.lineHeight = r.lineHeight || this.fontMetrics.fontSize || r.fontSize))
	}
	ensureCharacters(e) {
		var b, x
		const t = O.graphemeSegmenter(e)
			.filter((g) => !this._currentChars.includes(g))
			.filter((g, p, w) => w.indexOf(g) === p)
		if (!t.length) return
		...
		let l = this._currentX,
			u = this._currentY,
			d = this._currentMaxCharHeight
		const c = this.baseRenderedFontSize / this.baseMeasurementFontSize,
			h = this._padding * c
		let f = !1
		const y = s.width / this.resolution,
			_ = s.height / this.resolution
		for (let g = 0; g < t.length; g++) {
			const p = t[g],
				w = O.measureText(p, a, s, !1)
			w.lineHeight = w.height
			const P = w.width * c,
				G = Math.ceil((a.fontStyle === 'italic' ? 2 : 1) * P),
				A = w.height * c,
				U = G + h * 2,
				V = A + h * 2
			if (
...
` &&
					...
					p !== '	' &&
					p !== ' ' &&
					((f = !0), (d = Math.ceil(Math.max(V, d)))),
				l + U > y && ((u += d), (d = V), (l = 0), u + d > _))
			) {
				i.update()
			...
			const ct =
				P / c -
				(((b = a.dropShadow) == null ? void 0 : b.distance) ?? 0) -
				(((x = a._stroke) == null ? void 0 : x.width) ?? 0)
			if (
				((this.chars[p] = {
					id: p.codePointAt(0),
				...
				this._drawGlyph(n, w, l + h, u + h, c, a)
				const z = i.width * c,
					he = i.height * c,
					dt = new pt(
						(l / z) * i.width,
				...
				;((this.chars[p].texture = new S({ source: i, frame: dt })), (l += Math.ceil(U)))
			}
		}
		;(i.update(),
			(this._currentX = l),
			(this._currentY = u),
			(this._currentMaxCharHeight = d),
	...
	_applyKerning(e, t) {
		...
		for (let s = 0; s < e.length; s++) {
			const n = e[s]
			for (let i = 0; i < this._currentChars.length; i++) {
				const a = this._currentChars[i]
				let l = r[n]
				l || (l = r[n] = t.measureText(n).width)
				let u = r[a]
				u || (u = r[a] = t.measureText(a).width)
				let d = t.measureText(n + a).width,
					c = d - (l + u)
				;(c && (this.chars[n].kerning[a] = c),
					(d = t.measureText(n + a).width),
					(c = d - (l + u)),
	...
	_setupContext(e, t, r) {
		...
		if (
			(s &&
				...
				(e.strokeStyle = ge(s, e))),
			t._fill && (e.fillStyle = ge(t._fill, e)),
			t.dropShadow)
		) {
			const i = t.dropShadow,
				a = Fe.shared.setValue(i.color).toArray(),
				l = i.blur * r,
				u = i.distance * r
			;((e.shadowColor = `rgba(${a[0] * 255},${a[1] * 255},${a[2] * 255},${i.alpha})`),
				(e.shadowBlur = l),
				(e.shadowOffsetX = Math.cos(i.angle) * u),
				(e.shadowOffsetY = Math.sin(i.angle) * u))
		} else
			((e.shadowColor = 'black'), (e.shadowBlur = 0), (e.shadowOffsetX = 0), (e.shadowOffsetY = 0))
	}
	_drawGlyph(e, t, r, s, n, i) {
		const a = t.text,
			l = t.fontProperties,
			u = i._stroke,
			d = ((u == null ? void 0 : u.width) ?? 0) * n,
			c = r + d / 2,
			h = s - d / 2,
			f = l.descent * n,
			y = t.lineHeight * n
		let _ = !1
		i.stroke && d && ((_ = !0), e.strokeText(a, c, h + y - f))
		const { shadowBlur: b, shadowOffsetX: x, shadowOffsetY: g } = e
		;(i._fill &&
			(_ && ((e.shadowBlur = 0), (e.shadowOffsetX = 0), (e.shadowOffsetY = 0)),
			e.fillText(a, c, h + y - f)),
			_ && ((e.shadowBlur = b), (e.shadowOffsetX = x), (e.shadowOffsetY = g)))
	}
	destroy() {
		super.destroy()
...
We.defaultOptions = { textureSize: 512, style: new E(), mipmap: !0 }
let me = We
function Ve(o, e, t, r) {
	const s = {
		width: 0,
	...
	const l = { width: 0, start: 0, index: 0, positions: [], chars: [] },
		...
		d = e.letterSpacing * u,
		c = e.wordWrapWidth * u,
		h = e.lineHeight ? e.lineHeight * u : t.lineHeight,
		f = e.wordWrap && e.breakWords,
		y = (x) => {
			const g = n.width
			for (let p = 0; p < l.index; p++) {
				const w = x.positions[p]
				;(n.chars.push(x.chars[p]), n.charPositions.push(w + g))
			}
		...
		_ = () => {
			...
			if (r) {
				let g = n.chars[x]
				for (; g === ' '; ) ((n.width -= t.chars[g].xAdvance), (g = n.chars[--x]))
			}
...
` ||
			...
			if (
				(!a && e.wordWrap && b(n.width + l.width)
					? (_(), y(l), p || n.charPositions.push(0))
					: ((l.start = n.width), y(l), p || n.charPositions.push(0)),
				g === '\r' ||
					g ===
						`
`)
				...
				const U = w.xAdvance + (w.kerning[i] || 0) + d
				;((n.width += U),
					(n.spaceWidth = U),
			...
			const A = w.kerning[i] || 0,
				U = w.xAdvance + A + d
			;(f && b(n.width + l.width + U) && (y(l), _()),
				(l.positions[l.index++] = l.width + A),
				l.chars.push(g),
				(l.width += U))
		}
		i = g
	...
	return (
		_(),
		e.align === 'center' ? Bt(s) : e.align === 'right' ? Ut(s) : e.align === 'justify' && Gt(s),
		s
	)
}
function Bt(o) {
...
function Gt(o) {
	...
	for (let t = 0; t < o.lines.length; t++) {
		...
		for (let d = 0; d < r.charPositions.length; d++)
			(d === n && ((n = r.spacesIndex[s++]), (i += u)), (r.charPositions[d] += i))
	}
}
function zt(o) {
	if (o === '') return []
	...
	for (let t = 0, r = o.length; t < r; t++) {
		...
		if (Array.isArray(s)) {
			...
			const n = s[0].charCodeAt(0),
				i = s[1].charCodeAt(0)
			if (i < n) throw new Error('[BitmapFont]: Invalid character range.')
			for (let a = n, l = i; a <= l; a++) e.push(String.fromCharCode(a))
...
class kt {
	...
	getFont(e, t) {
		...
		if (
			(t._fill.fill && !t._stroke
				...
				: (t._stroke || t.dropShadow) && ((r = `${t.styleKey}-bitmap`), (s = !1)),
			!F.has(r))
		) {
			const a = Object.create(t)
			a.lineHeight = 0
			const l = new me({ style: a, overrideFill: s, overrideSize: !0, ...this.defaultOptions })
			;(H++,
				H > 50 &&
					K(
						'BitmapText',
						`You have dynamically created ${H} bitmap fonts, this can be inefficient. Try pre installing your font styles using \`BitmapFont.install({name:"style1", style})\``
					),
				l.once('destroy', () => {
					;(H--, F.remove(r))
				}),
	...
	install(...e) {
		...
		const s = t.style,
			n = s instanceof E ? s : new E(s),
			i = t.dynamicFill ?? this._canUseTintForStyle(n),
			a = new me({
				style: n,
				overrideFill: i,
			...
			l = zt(t.chars)
		return (
			a.ensureCharacters(l.join('')),
			F.set(`${r}-bitmap`, a),
			a.once('destroy', () => F.remove(`${r}-bitmap`)),
			a
		)
	}
	uninstall(e) {
...
class It extends xt {
	constructor() {
		const e = Me.from({
				vertex: { source: xe, entryPoint: 'mainVertex' },
				fragment: { source: xe, entryPoint: 'mainFragment' },
				name: 'passthrough-filter'
			}),
			t = Re.from({ vertex: Dt, fragment: Ot, name: 'passthrough-filter' })
...
Le.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'filter' }
const _e = new B()
function Wt(o, e) {
	e.clear()
	const t = e.matrix
	for (let r = 0; r < o.length; r++) {
		const s = o[r]
		if (s.globalDisplayStatus < 7) continue
		const n = s.renderGroup ?? s.parentRenderGroup
		;(n != null && n.isCachedAsTexture
			? (e.matrix = _e.copyFrom(n.textureOffsetInverseTransform).append(s.worldTransform))
			: n != null && n._parentCacheAsTextureRenderGroup
				? (e.matrix = _e
						.copyFrom(n._parentCacheAsTextureRenderGroup.inverseWorldTransform)
						.append(s.groupTransform))
				: (e.matrix = s.worldTransform),
...
class He {
	constructor(e) {
		;((this._filterStackIndex = 0),
			(this._filterStack = []),
			(this._filterGlobalUniforms = new k({
			...
			(this._globalFilterBindGroup = new Be({})),
			(this.renderer = e))
	}
	...
	push(e) {
		...
		if (
			...
			this._calculateFilterBounds(s, t.renderTarget.rootViewPort, a, i, 1),
			s.skip)
		)
			return
		const u = this._getPreviousFilterData(),
			d = this._findFilterResolution(i)
		let c = 0,
			h = 0
		;(u && ((c = u.bounds.minX), (h = u.bounds.minY)),
			this._calculateGlobalFrame(s, c, h, d, n.width, n.height),
	...
	generateFilteredTexture({ texture: e, filters: t }) {
		...
		const s = e.source,
			n = s.resolution,
			i = s.antialias
		if (t.every((f) => !f.enabled)) return ((r.skip = !0), e)
		const a = r.bounds
		if ((a.addRect(e.frame), this._calculateFilterBounds(r, a.rectangle, i, n, 0), r.skip)) return e
		const l = n
		;(this._calculateGlobalFrame(r, 0, 0, l, s.width, s.height),
			(r.outputRenderSurface = M.getOptimalTexture(a.width, a.height, r.resolution, r.antialias)),
			(r.backTexture = S.EMPTY),
			(r.inputTexture = e),
	...
	pop() {
		...
		t.skip ||
			...
			t.blendRequired && M.returnTexture(t.backTexture),
			M.returnTexture(t.inputTexture))
	}
	getBackTexture(e, t, r) {
		const s = e.colorTexture.source._resolution,
			n = M.getOptimalTexture(t.width, t.height, s, !1)
		let i = t.minX,
			a = t.minY
		;(r && ((i -= r.minX), (a -= r.minY)), (i = Math.floor(i * s)), (a = Math.floor(a * s)))
	...
	applyFilter(e, t, r, s) {
		const n = this.renderer,
			...
			l = i.outputRenderSurface === r,
			u = n.renderTarget.rootRenderTarget.colorTexture.source._resolution,
			d = this._findFilterResolution(u)
		let c = 0,
			h = 0
		if (l) {
		...
		this._updateFilterUniforms(t, r, i, c, h, d, l, s)
	...
	calculateSpriteMatrix(e, t) {
		...
		return (
			i && i.cacheToLocalTransform && n.prepend(i.cacheToLocalTransform),
			n.invert(),
			s.prepend(n),
	...
	_getPassthroughFilter() {
		return (
			this._passthroughFilter ?? (this._passthroughFilter = new It()),
	...
	_setupBindGroupsAndRender(e, t, r) {
		...
		;(this._globalFilterBindGroup.setResource(t.source, 1),
			...
			r.encoder.draw({ geometry: Et, shader: e, state: e._state, topology: 'triangle-list' }),
			r.type === ae.WEBGL && r.renderTarget.finishRenderPass())
	}
	_setupFilterTextures(e, t, r, s) {
		if (
			((e.backTexture = S.EMPTY),
			(e.inputTexture = M.getOptimalTexture(t.width, t.height, e.resolution, e.antialias)),
			e.blendRequired)
		) {
			r.renderTarget.finishRenderPass()
			const n = r.renderTarget.getRenderTarget(e.outputRenderSurface)
			e.backTexture = this.getBackTexture(n, t, s == null ? void 0 : s.bounds)
		}
		;(r.renderTarget.bind(e.inputTexture, !0), r.globalUniforms.push({ offset: t }))
	}
	_calculateGlobalFrame(e, t, r, s, n, i) {
		const a = e.globalFrame
		;((a.x = t * s), (a.y = r * s), (a.width = n * s), (a.height = i * s))
	}
	_updateFilterUniforms(e, t, r, s, n, i, a, l) {
		const u = this._filterGlobalUniforms.uniforms,
			...
			y = u.uGlobalFrame,
			_ = u.uOutputTexture
		;(a ? ((d[0] = r.bounds.minX - s), (d[1] = r.bounds.minY - n)) : ((d[0] = 0), (d[1] = 0)),
			(d[2] = e.frame.width),
			(d[3] = e.frame.height),
			(c[0] = e.source.width),
			(c[1] = e.source.height),
			...
			(f[0] = 0.5 * h[2]),
			(f[1] = 0.5 * h[3]),
			(f[2] = e.frame.width * c[2] - 0.5 * h[2]),
			(f[3] = e.frame.height * c[3] - 0.5 * h[3]))
		const b = this.renderer.renderTarget.rootRenderTarget.colorTexture
		;((y[0] = s * i),
			(y[1] = n * i),
			(y[2] = b.source.width * i),
			(y[3] = b.source.height * i),
			t instanceof S && (t.source.resource = null))
	...
	_applyFiltersToTexture(e, t) {
		...
		if (
			...
			n[i].apply(this, r, e.outputRenderSurface, t)
		else {
			let l = e.inputTexture
			const u = M.getOptimalTexture(s.width, s.height, l.source._resolution, !1)
			let d = u
			for (let c = i; c < a; c++) {
				const h = n[c]
			...
			;(n[a].apply(this, l, e.outputRenderSurface, t), M.returnTexture(u))
		}
	}
	_calculateFilterBounds(e, t, r, s, n) {
		...
		for (let g = 0; g < l.length; g++) {
			...
			if (
				...
				(d += p.padding),
				p.antialias === 'off' ? (c = !1) : p.antialias === 'inherit' && c && (c = r),
				p.clipToViewport || (y = !1),
				!!!(p.compatibleRenderers & i.type))
			) {
				f = !1
				break
			}
			if (p.blendRequired && !(((x = i.backBuffer) == null ? void 0 : x.useBackBuffer) ?? !0)) {
				;(K(
		...
		if (
			(y && a.fitBounds(0, t.width / s, 0, t.height / s),
			a
				.scale(u)
				.ceil()
				.scale(1 / u)
		...
		;((e.antialias = c),
			(e.resolution = u),
			(e.blendRequired = h),
			(e.firstEnabledIndex = _),
			(e.lastEnabledIndex = b))
	}
	_popFilterData() {
	...
	_pushFilterData() {
		...
		return (
			e || (e = this._filterStack[this._filterStackIndex] = new Vt()),
			this._filterStackIndex++,
			e
		)
	}
}
He.extension = { type: [m.WebGLSystem, m.WebGPUSystem], name: 'filter' }
class N extends _t {
	constructor(e) {
		...
		;(super({ label: 'Graphics', ...s }),
			...
			t ? (this._context = t) : (this._context = this._ownedContext = new D()),
	...
	destroy(e) {
		;(this._ownedContext && !e
			...
			: (e === !0 || (e == null ? void 0 : e.context) === !0) && this._context.destroy(e),
	...
	clone(e = !1) {
		return e ? new N(this._context.clone()) : ((this._ownedContext = null), new N(this._context))
	}
	lineStyle(e, t, r) {
		v(
			T,
	...
	endFill() {
		...
		return (
			(e.width !== D.defaultStrokeStyle.width ||
				e.color !== D.defaultStrokeStyle.color ||
				e.alpha !== D.defaultStrokeStyle.alpha) &&
				this.context.stroke(),
			this
		)
	}
	drawCircle(...e) {
		return (
			v(T, 'Graphics#drawCircle has been renamed to Graphics#circle'),
			this._callContextMethod('circle', e)
		)
	}
	drawEllipse(...e) {
		return (
			v(T, 'Graphics#drawEllipse has been renamed to Graphics#ellipse'),
			this._callContextMethod('ellipse', e)
		)
	}
	drawPolygon(...e) {
		return (
			v(T, 'Graphics#drawPolygon has been renamed to Graphics#poly'),
			this._callContextMethod('poly', e)
		)
	}
	drawRect(...e) {
		return (
			v(T, 'Graphics#drawRect has been renamed to Graphics#rect'),
			this._callContextMethod('rect', e)
		)
	}
	drawRoundedRect(...e) {
		return (
			v(T, 'Graphics#drawRoundedRect has been renamed to Graphics#roundRect'),
			this._callContextMethod('roundRect', e)
		)
	}
	drawStar(...e) {
		return (
			v(T, 'Graphics#drawStar has been renamed to Graphics#star'),
			this._callContextMethod('star', e)
		)
	}
}
const Ye = class Xe extends ie {
	constructor(...e) {
		let t = e[0] ?? {}
		;(t instanceof Float32Array &&
			(v(T, 'use new MeshGeometry({ positions, uvs, indices }) instead'),
...
Ye.defaultOptions = { topology: 'triangle-list', shrinkBuffersToFit: !1 }
let ce = Ye
const ye = 'http://www.w3.org/2000/svg',
	be = 'http://www.w3.org/1999/xhtml'
class Ke {
	constructor() {
		;((this.svgRoot = document.createElementNS(ye, 'svg')),
			(this.foreignObject = document.createElementNS(ye, 'foreignObject')),
			(this.domElement = document.createElementNS(be, 'div')),
			(this.styleElement = document.createElementNS(be, 'style')))
...
function Lt(o, e, t, r) {
	r || (r = ve || (ve = new Ke()))
	const { domElement: s, styleElement: n, svgRoot: i } = r
	;((s.innerHTML = `<style>${e.cssStyle};</style><div style='padding:0'>${o}</div>`),
...
class $e {
	...
	execute(e) {
		...
		;((i.uTransformMatrix = e.groupTransform),
			(i.uRound = t._roundPixels | e._roundPixels),
			q(e.groupColorAlpha, i.uColor, 0),
	...
	_updateBatchesForRenderable(e, t) {
		...
		t.batches = s.batches.map((i) => {
			const a = j.get(yt)
			return (i.copyTo(a), (a.renderable = e), (a.roundPixels = n), a)
		})
	}
	destroy() {
...
$e.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'graphics' }
const je = class Ne extends ce {
	constructor(...e) {
		super({})
		let t = e[0] ?? {}
		;(typeof t == 'number' &&
	...
	build(e) {
		...
		const t = this.verticesX * this.verticesY,
			...
			u = this.height / a
		for (let c = 0; c < t; c++) {
			const h = c % this.verticesX,
				f = (c / this.verticesX) | 0
			;(r.push(h * l, f * u), s.push(h / i, f / a))
		}
		const d = i * a
		for (let c = 0; c < d; c++) {
			const h = c % i,
				f = (c / i) | 0,
				y = f * this.verticesX + h,
				_ = f * this.verticesX + h + 1,
				b = (f + 1) * this.verticesX + h,
...
je.defaultOptions = { width: 100, height: 100, verticesX: 10, verticesY: 10 }
let Yt = je
class de {
	constructor() {
	...
	get uvs() {
		...
		return (
			n.isSimple ||
				...
				(this._textureMatrixUpdateId !== n._updateID || this._uvUpdateId !== t._updateID) &&
					((!s || s.length < r.length) && (s = this._transformedUvs = new Float32Array(r.length)),
...
class qe {
	constructor(e, t) {
		;((this.localUniforms = new k({
			...
			(this.localUniformsBindGroup = new Be({ 0: this.localUniforms })),
			(this.renderer = e),
			(this._adaptor = t),
			this._adaptor.init())
	}
	validateRenderable(e) {
		...
		if (s) {
			...
			return (
				i.texture.uid !== e._texture.uid && (i._textureMatrixUpdateId = -1),
	...
	addRenderable(e, t) {
		var n, i
		const r = this.renderer.renderPipes.batch,
			s = this._getMeshData(e)
		if (
			(e.didViewUpdate &&
				((s.indexSize = (n = e._geometry.indices) == null ? void 0 : n.length),
				(s.vertexSize = (i = e._geometry.positions) == null ? void 0 : i.length)),
			s.batched)
		) {
	...
	execute(e) {
		if (!e.isRenderable) return
		e.state.blendMode = le(e.groupBlendMode, e.texture._source)
	...
	_getMeshData(e) {
		var t, r
		return (
			(t = e._gpuData)[(r = this.renderer.uid)] || (t[r] = new Te()),
...
qe.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'mesh' }
class Xt {
	execute(e, t) {
		const r = e.state,
			s = e.renderer,
		...
		const i = s.gl,
			a = e.getBuffers(t)
		;(s.shader.bind(n), s.state.set(r), s.geometry.bind(a.geometry, n.glProgram))
		const u =
			a.geometry.indexBuffer.data.BYTES_PER_ELEMENT === 2 ? i.UNSIGNED_SHORT : i.UNSIGNED_INT
...
function $t(o) {
	return { dynamicUpdate: Ce(o, !0), staticUpdate: Ce(o, !1) }
}
function Ce(o, e) {
	const t = []
	t.push(`
...
class jt {
	constructor(e) {
		...
		for (const d in r) {
			const c = r[d],
				h = se(c.format)
			c.dynamic ? (n += h.stride) : (s += h.stride)
		}
		;((this._dynamicStride = n / 4),
			(this._staticStride = s / 4),
			(this.staticAttributeBuffer = new L(t * 4 * s)),
			(this.dynamicAttributeBuffer = new L(t * 4 * n)),
			(this.indexBuffer = we(t)))
		const i = new ie()
		let a = 0,
			l = 0
		...
		for (const d in r) {
			const c = r[d],
				h = se(c.format)
			c.dynamic
				? (i.addAttribute(c.attributeName, {
	...
	update(e, t) {
		e.length > this._size &&
			...
			(this.staticAttributeBuffer = new L(this._size * this._staticStride * 4 * 4)),
			(this.dynamicAttributeBuffer = new L(this._size * this._dynamicStride * 4 * 4)),
			(this.indexBuffer = we(this._size)),
...
class Jt extends ue {
	constructor() {
		const e = Re.from({ vertex: Qt, fragment: qt }),
			t = Me.from({
				fragment: { source: Se, entryPoint: 'mainFragment' },
				vertex: { source: Se, entryPoint: 'mainVertex' }
			})
		super({
			glProgram: e,
			gpuProgram: t,
			resources: {
				...
				uniforms: {
					...
					uColor: { value: new Fe(16777215), type: 'vec4<f32>' },
...
class Qe {
	...
	_initBuffer(e) {
		return (
			(e._gpuData[this.renderer.uid] = new jt({
	...
	execute(e) {
		...
		;(s.update(t, e._childrenDirty),
			(e._childrenDirty = !1),
			(n.blendMode = le(e.blendMode, e.texture._source)))
		const i = this.localUniforms.uniforms,
			a = i.uTranslationMatrix
		;(e.worldTransform.copyTo(a),
			a.prepend(r.globalUniforms.globalUniformData.projectionMatrix),
			(i.uResolution = r.globalUniforms.globalUniformData.resolution),
...
Je.extension = { type: [m.WebGLPipes], name: 'particle' }
class Ze extends Qe {
	constructor(e) {
		super(e, new Kt())
	}
}
Ze.extension = { type: [m.WebGPUPipes], name: 'particle' }
const et = class tt extends Yt {
	constructor(e = {}) {
	...
	updatePositions() {
		const e = this.positions,
			...
			b = u * r
		;((e[0] = e[8] = e[16] = e[24] = -_),
			(e[2] = e[10] = e[18] = e[26] = s * y - _),
			(e[4] = e[12] = e[20] = e[28] = t - n * y - _),
			(e[6] = e[14] = e[22] = e[30] = t - _),
			(e[1] = e[3] = e[5] = e[7] = -b),
			(e[9] = e[11] = e[13] = e[15] = i * y - b),
			(e[17] = e[19] = e[21] = e[23] = r - a * y - b),
			(e[25] = e[27] = e[29] = e[31] = r - b),
...
class rt {
	...
	_initGPUSprite(e) {
		const t = (e._gpuData[this._renderer.uid] = new er()),
			r = t
		return (
			(r.renderable = e),
...
rt.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'nineSliceSprite' }
const tr = {
		name: 'tiling-bit',
		vertex: {
			header: `
...
class sr extends ue {
	constructor() {
		;(J ?? (J = Ge({ name: 'tiling-sprite-shader', bits: [Ft, tr, ze] })),
			Z ?? (Z = ke({ name: 'tiling-sprite-shader', bits: [Mt, rr, Ae] })))
		const e = new k({
		...
		super({
			...
			resources: {
				...
				uSampler: S.EMPTY.source.style
			}
		})
	}
	updateUniforms(e, t, r, s, n, i) {
		const a = this.resources.tilingUniforms,
			l = i.width,
			u = i.height,
			d = i.textureMatrix,
			c = a.uniforms.uTextureTransform
		;(c.set((r.a * l) / e, (r.b * l) / t, (r.c * u) / e, (r.d * u) / t, r.tx / e, r.ty / t),
			c.invert(),
			(a.uniforms.uMapCoord = d.mapCoord),
			(a.uniforms.uClampFrame = d.uClampFrame),
			(a.uniforms.uClampOffset = d.uClampOffset),
			...
			i && ((this.resources.uTexture = i.source), (this.resources.uSampler = i.source.style)))
	}
}
class nr extends ce {
	constructor() {
...
function ar(o, e, t, r) {
	...
	for (t *= e; s < n; ) {
		const h = o[t],
			f = o[t + 1]
		;((o[t] = i * h + l * f + d), (o[t + 1] = a * h + u * f + c), (t += e), s++)
	}
}
function or(o, e) {
	const t = o.texture,
	...
	let n = 0,
		i = 0
	;(o.applyAnchorToTexture && ((n = o.anchor.x), (i = o.anchor.y)),
		(e[0] = e[6] = -n),
		(e[2] = e[4] = 1 - n),
		(e[1] = e[3] = -i),
		(e[5] = e[7] = 1 - i))
	const a = B.shared
	;(a.copyFrom(o._tileTransform.matrix),
		(a.tx /= o.width),
		(a.ty /= o.height),
		a.invert(),
		a.scale(o.width / r, o.height / s),
		ar(e, 2, 0, a))
}
const Y = new nr()
class lr {
...
class st {
	...
	addRenderable(e, t) {
		...
		if (i) {
			...
			;(e.didViewUpdate &&
				...
				r.addToBatch(a, t))
		} else (r.break(t), s.shader || (s.shader = new sr()), this.updateRenderable(e), t.add(e))
	}
	execute(e) {
		...
		;((r.uTransformMatrix = e.groupTransform),
			...
			(this._state.blendMode = le(e.groupBlendMode, e.texture._source)),
			this._renderer.encoder.draw({ geometry: Y, shader: t, state: this._state }))
	}
	updateRenderable(e) {
		const t = this._getTilingSpriteData(e),
			{ canBatch: r } = t
	...
	_updateCanBatch(e) {
		...
		let s = !0
		return (
			this._renderer.type === ae.WEBGL && (s = this._renderer.context.supports.nonPowOf2wrapping),
			(t.canBatch = r.textureMatrix.isSimple && (s || r.source.isPowerOfTwo)),
			t.canBatch
		)
	}
}
st.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'tilingSprite' }
const ur = {
...
class fr extends ue {
	constructor(e) {
		...
		;(ee ?? (ee = Ge({ name: 'sdf-shader', bits: [bt, vt(e), ur, dr, ze] })),
			te ?? (te = ke({ name: 'sdf-shader', bits: [Tt, wt(e), cr, hr, Ae] })),
			super({
				glProgram: te,
				gpuProgram: ee,
				resources: { localUniforms: t, batchSamplers: Ct(e) }
			}))
	}
}
class pr extends N {
...
class nt {
	...
	_updateContext(e, t) {
		const { context: r } = t,
			s = At.getFont(e.text, e._style)
		;(r.clear(),
			s.distanceField.type !== 'none' &&
				(r.customShader || (r.customShader = new fr(this._renderer.limits.maxBatchableTextures))))
		const n = O.graphemeSegmenter(e.text),
			i = e._style
		...
		;(i._stroke && ((c += i._stroke.width / d), (h += i._stroke.width / d)),
			r.translate(-e._anchor._x * c - u, -e._anchor._y * h - u).scale(d, d))
		const f = s.applyFillAsTint ? i._fill.color : 16777215
		let y = s.fontMetrics.fontSize,
			_ = s.lineHeight
		i.lineHeight && ((y = i.fontSize / d), (_ = i.lineHeight / d))
		let b = (_ - y) / 2
		b - s.baseLineOffset < 0 && (b = 0)
			...
			for (let p = 0; p < g.charPositions.length; p++) {
				...
				if (P != null && P.texture) {
					...
					r.texture(
						G,
						f || 'black',
						Math.round(g.charPositions[p] + P.xOffset),
						Math.round(a + P.yOffset + b),
						G.orig.width,
						G.orig.height
					)
				}
	...
	_updateDistanceField(e) {
		const t = this._getGpuBitmapText(e).context,
			...
			s = F.get(`${r}-bitmap`),
			{ a: n, b: i, c: a, d: l } = e.groupTransform,
			u = Math.sqrt(n * n + i * i),
			d = Math.sqrt(a * a + l * l),
			c = (Math.abs(u) + Math.abs(d)) / 2,
			h = s.baseRenderedFontSize / e._style.fontSize,
			f = c * s.distanceField.range * (1 / h)
...
nt.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'bitmapText' }
function Pe(o, e) {
	;((e.groupTransform = o.groupTransform),
		(e.groupColorAlpha = o.groupColorAlpha),
		(e.groupColor = o.groupColor),
		(e.groupBlendMode = o.groupBlendMode),
		(e.globalDisplayStatus = o.globalDisplayStatus),
		(e.groupTransform = o.groupTransform),
		(e.localDisplayStatus = o.localDisplayStatus),
		(e.groupAlpha = o.groupAlpha),
		(e._roundPixels = o._roundPixels))
}
class gr extends De {
	constructor(e) {
		;(super(),
...
function ne(o, e) {
	...
	St(r, e._anchor, t)
	const n = e._anchor._x * s * 2,
		i = e._anchor._y * s * 2
	;((r.minX -= s - n), (r.minY -= s - i), (r.maxX -= s - n), (r.maxY -= s - i))
}
class it {
	constructor(e) {
		this._renderer = e
	...
	addRenderable(e, t) {
		...
		if (e._didTextUpdate) {
			...
			;((r.currentKey !== e.styleKey || e.resolution !== s) &&
	...
	async _updateGpuText(e) {
		...
		;(n && (n.structureDidChange = !0), (t.generatingTexture = !1), ne(t, e))
	}
	_getGpuText(e) {
		return e._gpuData[this._renderer.uid] || this.initGpuText(e)
	}
...
it.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'htmlText' }
function mr() {
	const { userAgent: o } = oe.get().getNavigator()
...
function at(o, e, t, r) {
	...
	const n = M.getOptimalTexture(s.width, s.height, r, !1)
	return (
...
async function vr(o) {
	const e = o
		.filter((t) => F.has(`${t}-and-url`))
		.map((t) => {
			if (!re.has(t)) {
				const { entries: r } = F.get(`${t}-and-url`),
					s = []
				;(r.forEach((n) => {
					const i = n.url,
						l = n.faces.map((u) => ({ weight: u.weight, style: u.style }))
					s.push(
						...l.map((u) => br({ fontWeight: u.weight, fontStyle: u.style, fontFamily: t }, i))
					)
				}),
					re.set(
						t,
...
function Tr(o, e, t, r, s) {
	const { domElement: n, styleElement: i, svgRoot: a } = s
	;((n.innerHTML = `<style>${e.cssStyle}</style><div style='padding:0;'>${o}</div>`),
		n.setAttribute(
			'style',
...
function wr(o, e) {
	const t = X.getOptimalCanvasAndContext(o.width, o.height, e),
		{ context: r } = t
...
function Cr(o, e, t) {
	return new Promise(async (r) => {
		;(t && (await new Promise((s) => setTimeout(s, 100))),
			...
			(o.src = `data:image/svg+xml;charset=utf8,${encodeURIComponent(e)}`),
...
class ot {
	constructor(e) {
		;((this._activeTextures = {}),
			(this._renderer = e),
			(this._createCanvas = e.type === ae.WEBGPU))
	}
	getTexture(e) {
	...
	getManagedTexture(e) {
		...
		return ((this._activeTextures[t] = { texture: null, promise: r, usageCount: 1 }), r)
	}
	getReferenceCount(e) {
		var t
	...
	async _buildTexturePromise(e) {
		const { text: t, style: r, resolution: s, textureStyle: n } = e,
			i = j.get(Ke),
			a = _r(t, r),
			l = await vr(a),
			u = Lt(t, r, l, i),
			d = Math.ceil(Math.ceil(Math.max(1, u.width) + r.padding * 2) * s),
			c = Math.ceil(Math.ceil(Math.max(1, u.height) + r.padding * 2) * s),
			h = i.image,
			f = 2
		;((h.width = (d | 0) + f), (h.height = (c | 0) + f))
		const y = Tr(t, r, s, l, i)
		await Cr(h, y, mr() && a.length > 0)
		const _ = h
		let b
		this._createCanvas && (b = wr(h, s))
		const x = at(b ? b.canvas : _, h.width - f, h.height - f, s)
		return (
			n && (x.source.style = n),
			this._createCanvas &&
				(this._renderer.texture.initSource(x.source), X.returnCanvasAndContext(b)),
			j.return(i),
			x
		)
	}
	...
	_cleanUp(e) {
		;(M.returnTexture(e, !0), (e.source.resource = null), (e.source.uploadMethodId = 'unknown'))
	}
	destroy() {
		this._renderer = null
...
ot.extension = { type: [m.WebGLSystem, m.WebGPUSystem, m.CanvasSystem], name: 'htmlText' }
class Sr extends De {
	constructor(e) {
...
lt.extension = { type: [m.WebGLPipes, m.WebGPUPipes, m.CanvasPipes], name: 'text' }
class ut {
	constructor(e) {
	...
	getTexture(e, t, r, s) {
		...
		const { text: n, style: i, textureStyle: a } = e,
			l = e.resolution ?? this._renderer.resolution,
			{ frame: u, canvasAndContext: d } = Q.getCanvasAndContext({
				text: n,
				style: i,
				resolution: l
			}),
			c = at(d.canvas, u.width, u.height, l)
		if (
			(a && (c.source.style = a),
			i.trim && (u.pad(i.padding), c.frame.copyFrom(u), c.frame.scale(1 / l), c.updateUvs()),
			i.filters)
		) {
			const h = this._applyFilters(c, i.filters)
	...
	getManagedTexture(e) {
		...
		return ((this._activeTextures[t] = { texture: r, usageCount: 1 }), r)
	}
	decreaseReferenceCount(e) {
		const t = this._activeTextures[e]
		;(t.usageCount--,
			t.usageCount === 0 && (this.returnTexture(t.texture), (this._activeTextures[e] = null)))
	}
	getReferenceCount(e) {
		var t
	...
	_applyFilters(e, t) {
		const r = this._renderer.renderTarget.renderTarget,
			s = this._renderer.filter.generateFilteredTexture({ texture: e, filters: t })
...
ut.extension = { type: [m.WebGLSystem, m.WebGPUSystem, m.CanvasSystem], name: 'canvasText' }
C.add(Oe)
C.add(Ie)
C.add($e)
C.add(Pt)
